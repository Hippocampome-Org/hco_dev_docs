...
Note: NS has seen similar equation coding of Izhikevich model derivatives in pg. 3 code of [3]. Therefore, while it is unclear how it works, there seems to be evidence that derivatives processing can be computed in such a way. Izhikevich added a processing the derivative calculation twice per 1 ms timestep for "numerical stability" but perhaps that is optional.
</details>
<br>
